...
// Function to cluster waypoints by state, then by zip code
const clusterWaypointsByStateAndZip = (waypoints) => {
    const stateClusters = {};
    ...
    // For each state cluster, further cluster by zip code and sort by proximity
    Object.keys(stateClusters).forEach(state => {
        const zipClusters = {};

        stateClusters[state].forEach(wp => {
            if (!zipClusters[wp.zipCode]) {
                zipClusters[wp.zipCode] = [];
            }
            zipClusters[wp.zipCode].push(wp);
        });

        // Sort each zip cluster by proximity
        Object.keys(zipClusters).forEach(zipCode => {
            zipClusters[zipCode] = sortWaypointsByProximity(zipClusters[zipCode], startingLocation);
        });

        // Flatten zip clusters back into the state cluster
        stateClusters[state] = Object.values(zipClusters).flat();
    });

    return stateClusters;
};

// Function to get distance matrix in batches
const getDistanceMatrixInBatches = async (waypoints) => {
    const batchSize = 10; // Maximum number of origins/destinations per request
    const requests = [];
    
    for (let i = 0; i < waypoints.length; i += batchSize) {
        for (let j = 0; j < waypoints.length; j += batchSize) {
            const origins = waypoints.slice(i, i + batchSize);
            const destinations = waypoints.slice(j, j + batchSize);

            const params = {
                origins: origins.map(wp => `${wp.coordinates.lat},${wp.coordinates.lng}`).join('|'),
                destinations: destinations.map(wp => `${wp.coordinates.lat},${wp.coordinates.lng}`).join('|'),
...
app.post('/calculate-routes', async (req, res) => {
    ...
    try {
        ...
        // Get distance matrices in batches
        const distanceMatrices = await getDistanceMatrixInBatches(geocodedWaypoints);
        console.log('Distance matrices:', distanceMatrices);

        // Cluster and sort waypoints by state and then by zip
        const clusters = clusterWaypointsByStateAndZip(geocodedWaypoints);
